...
img = Image.open("test2.jpg") # Use image
...
if resizeImage:
    img = img.resize((640, 360)) #Go faster, lose image quality
    ...
    for row in range(nb_rows):
        for column in range(nb_columns):
            ...
            if matrix[c_row][c_column] == 1:
                ...
                matrix[c_row][c_column] = 200 # Make spotted pixels visible in end result
    return objects

def are_part_of_same_object(pixels, min):
...
def main(counter):
    ...
    while len(objects) > 3:
        objects = are_part_of_same_object(objects, min_nb_pixels)
        counter += 1
