...
class BROPAS(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.geometry("1050x600")  # start dimensions
...
class MainMenu(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # info button
        ib = tk.Button(self, text="Coming soon", width=11, relief="groove", borderwidth=2, command=lambda: messagebox.showinfo(None, "Coming soon..."))
        ib.grid(row=0, column=0, sticky="w", padx=10)
        # methode button
        mb = tk.Button(self, text="Method", width=13, relief="groove", borderwidth=2, command=lambda: messagebox.showinfo(None, "Coming soon..."))
        mb.grid(row=0, column=0, sticky="nes")

        # start button
        start_button = tk.Button(self, text="Start", width=10, height=3, bg="light steel blue", relief="groove", borderwidth=2,
...
class ScanScreen(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # via camera button:
        cb = tk.Button(self, text="Use Camera", relief="groove", borderwidth=2, font=LARGER_FONT, bg="light steel blue", height=2, width=20,
                       command=lambda: self.run_with_kinect())
        cb.grid(row=0, column=1, sticky="ew", padx=20, columnspan=1)

        # via existing picture button
        pb = tk.Button(self, text="Select Image", font=LARGER_FONT, relief="groove", bg="light steel blue", borderwidth=2, height=2, width=20,
                       command=lambda: self.run_on_selected_img())
        pb.grid(row=0, column=3, sticky="ew", padx=20, columnspan=1)
        ...
        # gauss label
        gauss_label = tk.Label(gaussFrame, text="Times blur:")
        ...
        show_cb = tk.Checkbutton(self, variable=self.show_bool, text="Show result", font=LARGE_FONT,
        ...
        fill_cb = tk.Checkbutton(self, variable=self.fill_bool, text="Fill object")
        fill_cb.select()
        fill_cb.grid(row=4, column=3, sticky="w")

        # sobel2 checkbox:
        self.senne_bool = tk.IntVar()
        sobel2_cb = tk.Checkbutton(self, variable=self.senne_bool, text="Object counting from scratch")
        #sobel2_cb.select()
        ...
        # include depth button
        self.depth = tk.IntVar()
        depth_cb = tk.Checkbutton(self, variable=self.depth, text="Include depth data")
        #depth_cb.select()
        depth_cb.grid(row=1, column=4, sticky="w")

        # initialize
        self.initialize = tk.IntVar()
        initialize_cb = tk.Checkbutton(self, variable=self.initialize, text="Initialize depth camera")
        # depth_cb.select()
        initialize_cb.grid(row=2, column=4, sticky="w")

        # back button
        bb = tk.Button(self, text="Back to menu", command=lambda: controller.show_frame(MainMenu))
        bb.grid(row=9, column=4, sticky="e")
    ...
    def run_on_selected_img(self):
        ...
        if self.keep_bool.get() and not self.keep_path.get() == "":
            ...
            depth_img_path = self.get_filepath()
    ...
    def color_pipeline(self, image, show_dict, save_dict):
        ...
        if self.senne_bool.get():
            ...
            filled = np.array(filled.resize((int(width / 5), int(height / 5))))
    ...
    def depth_pipeline(self, depth_image, image,  show_dict, save_dict):
        ...
        show_dict.update({"Grayscaled": (gray, 'gray')})
        save_dict.update(
            {"Gray": (gray, f'{ESAT7A1}/Images/grayscaled images/')})  # gray is the reference, don't change it
        if self.gauss_bool.get():
            gauss = wp.gaussian_blur(gray, self.gauss_reps.get())
            show_dict.update({"Gaussian Blur": (gauss, 'gray')})
            save_dict.update({"Gauss": (gauss, f'{ESAT7A1}/Images/blurred images/')})
        ...
        if overlap:
            ...
            show_dict.update({"Depth final result": (resultFrame2, 'viridis')})
